    ...
    public function get($itemId = 0)
    {
        ...
        if ($stmt->execute()) {
            ...
            while ($row = $stmt->fetch()) {
                ...
                $stmt2 = $this->db->prepare("SELECT id FROM photos WHERE fromUserId = (:fromUserId) AND moderateAt > 0 AND id < (:itemId) AND removeAt = 0 ORDER BY id DESC");
                $stmt2->bindParam(':fromUserId', $row['friend'], PDO::PARAM_INT);
            ...
            $stmt3 = $this->db->prepare("SELECT id FROM photos WHERE fromUserId = (:fromUserId) AND moderateAt > 0 AND id < (:itemId) AND removeAt = 0 ORDER BY id DESC");
