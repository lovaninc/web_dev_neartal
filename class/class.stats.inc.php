...
class stats extends db_connect
{
    ...
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM users WHERE photoPostModerateAt > 0 AND photoModerateAt = 0");
        $stmt->execute();
    ...
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM users WHERE coverPostModerateAt > 0 AND coverModerateAt = 0");
        $stmt->execute();
